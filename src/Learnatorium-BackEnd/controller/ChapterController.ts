...
export const UpdateChapter = async ({ request, response }: RouterContext) => {
  ...
  let chapFound = await Chapter.where("id", id).get();

  if (chapFound.toString().length > 2) {
    await Chapter.where("id", id).update({ group_name, author });
    response.status = 200;
...
export const UpdateRevision = async ({ request, response }: RouterContext) => {
  ...
  let last_revision = new Date().toISOString().slice(0, 19).replace("T", " ");
  await Chapter.where("id", id).update({ last_revision });
  response.body = await Chapter.where("id", id).get();
  response.status = 200;
};
...
export const DeleteChapter = async ({ params, response }: RouterContext) => {
  ...
  console.log(id);
  const chapterFound = await Chapter.where("id", id).get();

  if (chapterFound.toString().length > 2) {

    await Chapter.where("id", id).delete();
